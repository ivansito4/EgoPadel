...
namespace EgoPadel.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        {
            if(!await _rolManager.RoleExistsAsync(WC.AdminRol))  //Si no existe ningun rol, se crean
            {
                await _rolManager.CreateAsync(new IdentityRole(WC.AdminRol));
                await _rolManager.CreateAsync(new IdentityRole(WC.UsuarioRol));
            }
            ReturnUrl = returnUrl;
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                //var user = CreateUser();  Vamos a usar UsuarioApp
                ...
                await _userStore.SetUserNameAsync(user, Input.Login, CancellationToken.None); //antes era input.email
                ...
                if (result.Succeeded)   //Usuario creado correctamente
                {
                    ...
                    await _emailSender.SendEmailAsync(Input.Email, "Confirma tu email",
                        $"Porfavor confirma tu email haciendo click <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>aquí</a>.");
